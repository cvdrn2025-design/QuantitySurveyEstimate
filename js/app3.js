...
(function() {
    ...
    function cetakRingkasan() {
        ...
        if (project) {
            text += `Proyek: ${project.nama || '-'}\n`;
            text += `Lokasi: ${project.lokasi || '-'}\n`;
            text += `Klien: ${project.klien || '-'}\n`;
        }
        text += `Tanggal Cetak: ${dateStr} ${timeStr}\n`;
        ...
        if (state.gambarList.length > 0) {
            text += '\n--- ACUAN GAMBAR ---\n';
            state.gambarList.forEach((g, i) => {
                text += `${i+1}. ${g.name} (${(g.size/1024).toFixed(1)} KB)\n`;
            });
        }
        ...
        if (win) {
            win.document.write(`
                ...
                <style>
                    ...
                    .footer { text-align: center; margin-top: 30px; padding-top: 10px; border-top: 2px solid #2563eb; font-size: 11px; color: #64748b; }
                </style>
                </head>
                <body>
                    <pre>${text}</pre>
